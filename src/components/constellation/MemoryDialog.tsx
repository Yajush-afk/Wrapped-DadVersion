    ...
    <div className="memory-overlay" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="memory-dialog" role="dialog" aria-modal="true" aria-labelledby="memory-title" ref={panelRef}>
        ...
        <div className="memory-dialog__copy">
          <p className="memory-dialog__count">Lesson {position} / {total}</p>
          <h2 id="memory-title">{memory.title}</h2>
          <p>{memory.memory}</p>
          <nav className="memory-dialog__nav" aria-label="Lesson navigation">
            <button onClick={onPrevious} aria-label="Previous lesson">← Previous</button>
            <button onClick={onNext} aria-label="Next lesson">Next →</button>
          </nav>
        </div>
      </div>
    </div>
